...
1. Install @wemnyelezxnpm/soluta-eaque-minus:

   ```
   npm install --save-dev @wemnyelezxnpm/soluta-eaque-minus
   ```

2. Add @wemnyelezxnpm/soluta-eaque-minus to your ESLint configuration – either to [eslintrc] or to [eslint.config.js (flat config)].
   ...
   - eslint.config.js (flat config): Import @wemnyelezxnpm/soluta-eaque-minus, and put it in the configuration array – **after** other configs that you want to override.

     <!-- prettier-ignore -->
     ```js
     import someConfig from "some-other-config-you-use";
     import eslintConfigPrettier from "@wemnyelezxnpm/soluta-eaque-minus";
...
@wemnyelezxnpm/soluta-eaque-minus not only turns off _core_ rules, but also some from these plugins automatically:
...
> ℹ️ Note: You might find guides on the Internet saying you should also extend stuff like `"prettier/react"`. Since version 8.0.0 of @wemnyelezxnpm/soluta-eaque-minus, all you need to extend is `"prettier"`! That includes all plugins.
...
You might expect @wemnyelezxnpm/soluta-eaque-minus to turn off `ts/indent`, but it won’t! Because @wemnyelezxnpm/soluta-eaque-minus only turns off `@typescript-eslint/indent`. It cannot know what you chose to call the plugin. Same thing for the CLI helper tool.
...
Some of the rules that @wemnyelezxnpm/soluta-eaque-minus turns off may be deprecated, or even removed from ESLint. **This is perfectly fine,** but if you really need to omit the deprecated and removed rules, you can do so by setting the `ESLINT_CONFIG_PRETTIER_NO_DEPRECATED` environment variable to a non-empty value. For example:
...
@wemnyelezxnpm/soluta-eaque-minus also ships with a little CLI tool to help you check if your configuration contains any rules that are unnecessary or conflict with Prettier. Here’s how to run it:

```
npx @wemnyelezxnpm/soluta-eaque-minus path/to/main.js
```
...
With the new ESLint “flat config” format, you can control what things override what yourself. One way of solving the above conflict is to reorder the config objects so that @wemnyelezxnpm/soluta-eaque-minus is last:
...
export default [
  ...
  eslintConfigPrettier, // @wemnyelezxnpm/soluta-eaque-minus last
];
```

However, looking at the above config might feel confusing. It looks like we enable the `indent` rule, but in reality it’s disabled thanks to the `eslintConfigPrettier` line below it. Instead you might want to actually have your own rules _after_ @wemnyelezxnpm/soluta-eaque-minus and run the CLI helper tool to find out about problems, so you can remove conflicting rules from the config file altogether (simplifying your config).
...
```
npx @wemnyelezxnpm/soluta-eaque-minus index.js test/index.js legacy/main.js
```
...
Just like ESLint itself, you can control the @wemnyelezxnpm/soluta-eaque-minus CLI helper tool using the `ESLINT_USE_FLAT_CONFIG` environment variable:
...
@wemnyelezxnpm/soluta-eaque-minus versions before 7.0.0 had a slightly different CLI tool that was run in a different way. For example:

```
npx eslint --print-config index.js | npx @wemnyelezxnpm/soluta-eaque-minus-check
```

If you find something like that in a tutorial, this is what the command looks like in 7.0.0 or later:

```
npx @wemnyelezxnpm/soluta-eaque-minus index.js
```

## Special rules

There a few rules that @wemnyelezxnpm/soluta-eaque-minus disables that actually can be enabled in some cases.
...
See [package.json] for the exact versions of ESLint, Prettier and ESLint plugins that @wemnyelezxnpm/soluta-eaque-minus has been tested with.
...
`test-lint/foobar.js` must fail when used with eslint-plugin-foobar and eslint-plugin-prettier at the same time – until @wemnyelezxnpm/soluta-eaque-minus is added to the ESLint config. The file should be formatted according to Prettier, and that formatting should disagree with the plugin.
...
- `"test:eslint"` makes sure that the files in `test-lint/` pass ESLint when the exclusions from @wemnyelezxnpm/soluta-eaque-minus are used. It also lints the code of @wemnyelezxnpm/soluta-eaque-minus itself.
...
[package.json]: https://github.com/wemnyelezxnpm/soluta-eaque-minus/blob/main/package.json
